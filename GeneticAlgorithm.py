...
class GeneticAlgorithm:
    #niter =100
    # setting test_size from the point where we are calling the this
    def __init__(self,X,Y,Algorithm,test_size, Niter=10,keep_fraction=0.5,mutation_rate="auto",nfeatures="auto",njobs=4):
    ...
    def fitness(self,generation):
        ...
        def determine_fitness(subgeneration,output,pos):
            ...
            for i in range(np.shape(subgeneration)[0]):
                ...
                # we dont need shuffling as it was already shuffled.
                X_train, X_test, y_train, y_test = train_test_split(X_individual,self.response,test_size=self.test_size, shuffle=False, stratify=None)
